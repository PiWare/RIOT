...
class nvic
{
	...
	public:
		...
		struct irq_tag
		{
			static_assert(index_ <= 80, "Interrupt index out of range");
		...
		static void enable(const std::uint32_t value_low = 0, const std::uint32_t value_mid = 0, const std::uint32_t value_high = 0)
		{
			...
			if (config::index < 32)
			{
				if (value_low  | (1 << (config::index & 0x1F)))
					*(volatile std::uint32_t *)(BASE + ISER + 0x00) = value_low  | (1 << (config::index & 0x1F));

				if (value_mid)
					*(volatile std::uint32_t *)(BASE + ISER + 0x04) = value_mid;

				if (value_high)
					*(volatile std::uint32_t *)(BASE + ISER + 0x08) = value_high;
			}
			else
			if (config::index < 64)
			{
				if (value_low)
					*(volatile std::uint32_t *)(BASE + ISER + 0x00) = value_low;

				if (value_mid | (1ul << (config::index & 0x1F)))
					*(volatile std::uint32_t *)(BASE + ISER + 0x04) = value_mid | (1ul << (config::index & 0x1F));
			...
			else
			{
				...
				if (value_high | (1ul << (config::index & 0x1F)))
					*(volatile std::uint32_t *)(BASE + ISER + 0x08) = value_high | (1ul << (config::index & 0x1F));
			}
		}
		...
		static void enable(const std::uint32_t value_low = 0, const std::uint32_t value_mid = 0, const std::uint32_t value_high = 0)
		{
			...
			if (config::index < 32)
				nvic::enable<Tn, Args...>(value_low | (1 << (config::index & 0x1F)), value_mid, value_high);
			else
			if (config::index < 64)
				nvic::enable<Tn, Args...>(value_low, value_mid | (1 << (config::index & 0x1F)), value_high);
			else
				nvic::enable<Tn, Args...>(value_low, value_mid, value_high | (1u << (config::index & 0x1F)));
		}
